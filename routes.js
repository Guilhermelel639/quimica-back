...
router.get('/filme:id', async (req, res)=>{
    const { id } = req.params;
    try {
        const {recordset} = await pool.query`select * from filmes where ${id}`
        return res.status(200).json(recordset)
    } catch (error) {
...
router.get('/filmes/cidade:cidade', async (req, res)=>{
    ...
    try {
        const {recordset} = await pool.query`select * from filmescinema where ${cidade}`
        return res.status(200).json(recordset)
    } catch (error) {
        return res.status(500).json('error...')
    }
...
router.get('/sessao/:idU', async (req, res)=>{
    ...
    try {
        const {recordset} = await pool.query`select f.nome, s.horario, us.id_sessao from usuario_sessao as us
